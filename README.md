...
python3 next_code.py 3ppm3d -n 1000 --prefixes tv,tstats,tci --group-size 2 --backward
python3 -m unittest test_next_code.py
```

`-n 1000 --prefixes tv,tstats,tci --group-size 2` prints 1000 codes with `tv`, `tstats`, and `tci` prepended in that rotating order, a space before each code, and two lines per paste group (500 groups). Add `--backward` to walk toward earlier codes.
